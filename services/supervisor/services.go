...
func (sup *SupervisorService) start() error {
	...
	hostzktoroDir := os.Getenv(config.EnvHostzktoroDir)
	if len(hostzktoroDir) == 0 {
		return fmt.Errorf("supervisor needs to know $%s to mount to the other containers it runs", config.EnvHostzktoroDir)
	}
	...
	`),
			...
			Volumes: map[string]string{
				path.Join(hostzktoroDir, ".ipfs"): "/data/ipfs",
			},
	...
	if sup.config.Config.AdvancedConfig.IPFSExperiment {
		sup.storageContainer, err = sup.client.StartContainer(
			sup.ctx, docker.ContainerConfig{
				...
				Cmd:   []string{config.DefaultzktoroNodeBinaryPath, "storage"},
				...
				Volumes: map[string]string{
					...
					hostzktoroDir:          config.DefaultContainerzktoroDirPath,
				},
				Ports: map[string]string{
	...
	sup.jsonRpcContainer, err = sup.client.StartContainer(
		sup.ctx, docker.ContainerConfig{
			...
			Image: commonNodeImage,
			Cmd:   []string{config.DefaultzktoroNodeBinaryPath, "json-rpc"},
			Volumes: map[string]string{
				// give access to host docker
				"/var/run/docker.sock": "/var/run/docker.sock",
				hostzktoroDir:          config.DefaultContainerzktoroDirPath,
	...
	sup.publicAPIContainer, err = sup.client.StartContainer(
		sup.ctx, docker.ContainerConfig{
			...
			Image: commonNodeImage,
			Cmd:   []string{config.DefaultzktoroNodeBinaryPath, "public-api"},
			Volumes: map[string]string{
				// give access to host docker
				"/var/run/docker.sock": "/var/run/docker.sock",
				hostzktoroDir:          config.DefaultContainerzktoroDirPath,
	...
	sup.inspectorContainer, err = sup.client.StartContainer(
		sup.ctx, docker.ContainerConfig{
			...
			Image: commonNodeImage,
			Cmd:   []string{config.DefaultzktoroNodeBinaryPath, "inspector"},
			Volumes: map[string]string{
				hostzktoroDir: config.DefaultContainerzktoroDirPath,
			},
			Ports: map[string]string{
	...
	sup.scannerContainer, err = sup.client.StartContainer(
		sup.ctx, docker.ContainerConfig{
			...
			Image: commonNodeImage,
			Cmd:   []string{config.DefaultzktoroNodeBinaryPath, "scanner"},
			Env: map[string]string{
				config.EnvReleaseInfo: releaseInfo.String(),
			},
			Volumes: map[string]string{
	...
	sup.jwtProviderContainer, err = sup.client.StartContainer(
		sup.ctx, docker.ContainerConfig{
			...
			Image: commonNodeImage,
			Cmd:   []string{config.DefaultzktoroNodeBinaryPath, "jwt-provider"},
			Env: map[string]string{
				config.EnvReleaseInfo: releaseInfo.String(),
			},
			Volumes: map[string]string{
...
func manageIpfsDir(cfg config.Config) error {
	if !cfg.AdvancedConfig.IPFSExperiment {
		...
		os.RemoveAll(path.Join(config.DefaultContainerzktoroDirPath, ".ipfs"))
		return nil
	}
	if err := os.MkdirAll(path.Join(config.DefaultContainerzktoroDirPath, ".ipfs"), 0700); err != nil {
...
func (sup *SupervisorService) removeOldContainers() error {
	...
	for _, container := range containerList {
		...
		if !strings.Contains(containerName, "zktoro-agent-") {
			continue
		}
