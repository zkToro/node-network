...
import (
	...
	"zktoro/zktoro-core-go/contracts/merged/contract_zktoro_staking"
	"zktoro/zktoro-core-go/utils/ethutils"
...
type Contracts struct {
	...
	ZktoroStaking    *contract_zktoro_staking.ZktoroStakingCaller
	ZktoroStakingFil *contract_zktoro_staking.ZktoroStakingFilterer
...
func findContractAddress(regContracts map[string]interface{}, contractName string) common.Address {

	contract := regContracts[contractName].(map[string]interface{})
...
func NewClientWithoutENS(ctx context.Context, cfg ClientConfig) (*client, error) {
	regContractsTmp, err := ReadContractAddresses()

	var regContracts registry.RegistryContracts

	regContracts.AgentRegistry = findContractAddress(regContractsTmp, "agent-registry")
	regContracts.ScannerRegistry = findContractAddress(regContractsTmp, "scanner-registry")
	regContracts.ScannerPoolRegistry = findContractAddress(regContractsTmp, "scanner-pool-registry")
	regContracts.Dispatch = findContractAddress(regContractsTmp, "dispatch")
	...
	cl.contracts.ZktoroStakingFil, err = contract_zktoro_staking.NewZktoroStakingFilterer(regContracts.ZktoroStaking, ec)
	if err != nil {
		return nil, err
	}
	cl.versionManager.SetUpdateRule("ZktoroStaking", cl.contracts.ZktoroStaking, cl.contracts.ZktoroStaking, cl.contracts.ZktoroStakingFil)
...
func (c *client) GetActiveAgentStake(blockNumber *big.Int, botID string) (*big.Int, error) {
	...
	return c.contracts.ZktoroStaking.ActiveStakeFor(opts, SubjectTypeAgent, bID)
}
...
func (c *client) GetActiveScannerStake(blockNumber *big.Int, scannerID string) (*big.Int, error) {
	...
	sID := utils.ScannerIDHexToBigInt(scannerID)
	return c.contracts.ZktoroStaking.ActiveStakeFor(opts, SubjectTypeScanner, sID)
}
...
func (c *client) GenerateScannerRegistrationSignature(reg *eip712.ScannerNodeRegistration) (*ScannerRegistrationInfo, error) {
	...
	fmt.Println("ScannerPoolRegistry", c.contracts.Addresses.ScannerPoolRegistry)
...
func (c *client) GetActivePoolStake(blockNumber, poolID *big.Int) (*big.Int, error) {
	...
	opts := c.getBlockOpts(blockNumber)
	poolStake, err := contracts.ZktoroStaking.ActiveStakeFor(opts, SubjectTypeScannerPool, poolID)
	if err != nil {
		return nil, err
	}
	delegatorPoolStake, err := contracts.ZktoroStaking.ActiveStakeFor(opts, SubjectTypeDelegatorScannerPool, poolID)
	if err != nil {
		return nil, err
	}
