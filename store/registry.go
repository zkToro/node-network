...
func (rs *privateRegistryStore) GetAgentsIfChanged(scanner string) ([]config.AgentConfig, bool, error) {
	...
	for i, agentImage := range rs.cfg.LocalModeConfig.BotImages {
		...
		// zktoro-agent-1, zktoro-agent-2, zktoro-agent-3, ...
...
func GetRegistryClientWithoutENS(ctx context.Context, cfg config.Config, registryClientCfg registry.ClientConfig) (registry.Client, error) {
	return registry.NewClientWithoutENS(ctx, registryClientCfg)
}
